...
def computeProbaCPairsForState(strategies, init_state, nb_states, trans_states_proba):
    n = len(strategies)
    ...
    for i in range (n):
        ...
        if strat_1 == "TFT":
            ...
            strat_state = modifyStratFormat(strat_1)
            proba_c_pairs[i, i] = computeSingleProbaForState(strat_state, init_state, nb_states, rounds, trans_states_proba)
        for j in range(i+1, n):

            start_state = 0
            tft_proba_st0 = 1
            #proba_st0 = 1
            strat_state = modifyStratFormat(strategies[j])
            root_1 = Tree(start_state, strat_state, 1, nb_states - 1, trans_states_proba)
            proba_tft_st, proba_st = buildTreeTFTAgainstLRCD(tft_proba_st0, root_1, rounds)

            final_proba_tft = proba_tft_st
            final_proba_st = proba_st
            proba_c_pairs[i, j] = final_proba_tft
            proba_c_pairs[j, i] = final_proba_st
    return proba_c_pairs


def computeProbaCPairs(strategies, nb_states, trans_states_proba):
    n = len(strategies)
    ...
    for i in range (n):
        ...
        if strat_1 == "TFT":
            ...
            strat_state = modifyStratFormat(strat_1)
            proba_c_pairs[i, i] = computeSingleProba(strat_state, nb_states, rounds, trans_states_proba)
        for j in range(i+1, n):
            ...
            strat_state = modifyStratFormat(strategies[j])
            root_1 = Tree(start_state, strat_state, 1, nb_states - 1, trans_states_proba)
            proba_tft_st1, proba_st1 = buildTreeTFTAgainstLRCD(tft_proba_st0, root_1, rounds)

            final_proba_tft = (tft_proba_st0 + proba_tft_st1)/2
            final_proba_st = (proba_st0 + proba_st1)/2
            proba_c_pairs[i, j] = final_proba_tft
            proba_c_pairs[j, i] = final_proba_st
    return proba_c_pairs


def buildTreeTFTAgainstLRCD(init_proba_tft, root_1, rounds):

    proba_c1 = root_1.getProbaC()
    proba_tft = init_proba_tft

    root_1.addChildren(proba_tft)
    ...
    return np.clip(proba_tft, 0., 1.), np.clip(proba_c1, 0., 1.)


def computeSingleProbaForState(strat, init_state, nb_states, rounds, trans_states_proba):
    """
    Compute probability of cooperation for strategy using internal state in monomorphic scenario
    :param strat: strategy
    :param init_state: initial state value
    :param nb_states: number of states
    :param rounds: number of rounds
    :param trans_states_proba: list of transition probabilities for each state
    :return: probability of cooperating
    """
    init_weight = 1
    proba_c = 0
...
#proba_c_pairs = computeProbaCPairs(strategies, nb_states, trans_states_proba)
proba_c_pairs = computeProbaCPairsForState(strategies, 1, nb_states, trans_states_proba)
payoffs = computePayoffsPairs(R, S, T, P, nb_strats, proba_c_pairs)
t, f, stationary = stationaryDistrib(strategies, payoffs)
print(proba_c_pairs)
print(payoffs)
for i in range(len(strategies)):
    print(strategies[i], " : ", round(stationary[i], 8))
