    ...
    def addChildren(self, proba_c2):
        ...
        if new_state_c != self.current_state and proba_c2 > 0:
            if new_state_d != self.current_state and (1 - proba_c2) > 0:           #Both new_states are possible
                #weight_c = round(Tree.static_rho * proba_c2 * self.getWeight(), 10)
                #weight_d = round(Tree.static_rho * (1-proba_c2) * self.getWeight(), 10)
                #weight_no_change = round(round(1 - Tree.static_rho, 1) * round(1 - Tree.static_rho, 1)* self.getWeight(), 10)
                ...
                #weight_c = round(Tree.static_rho * proba_c2 * self.getWeight(),10)
                #weight_no_change = round(round((1 - proba_c2) + ((1 - Tree.static_rho) * proba_c2),10)* self.getWeight(), 10)      #(Proba(defect) + proba(cooperate and no change)) * pre-weight
                trans_c = self.getRhoForStates(new_state_c)
                weight_c = round(trans_c * proba_c2 * self.getWeight(),10)
            ...
            #weight_d = round(Tree.static_rho * (1 - proba_c2) * self.getWeight(),10)
            #weight_no_change = round(round(proba_c2 + (1 - Tree.static_rho) * (1 - proba_c2), 10) * self.getWeight(), 10)       #(Proba(cooperate) + proba(defect and no change)) * pre-weight
            trans_d = self.getRhoForStates(new_state_d)
            weight_d = round(trans_d * (1 - proba_c2) * self.getWeight(),10)
